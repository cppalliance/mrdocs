...
| `anchor`
| `string`
| The symbol's own (unqualified) URL- and filename-safe legible name. Generators join these along the parent chain to form a page-unique id or path.

| `parent`
| `string`
| The id of the symbol's parent context (namespace or record), or absent for the global namespace. Resolve it to a Symbol with `@root.mrdocs.corpus.get`.
...
Handlebars generators expose corpus functions that compute a symbol's
links. Call them with a symbol object, for example
`(@root.mrdocs.corpus.getUrl symbol)`:
...
| `getUrl`
| `string`
| The symbol's URL: a `#`-prefixed fragment on a single page, or a `/`-prefixed path (with the file extension) in multipage output.

| `qualifiedAnchor`
| `string`
| The symbol's page-unique anchor id: the qualified legible name joined with `-`, used as a heading or section id. (The per-symbol `anchor` field above is only the unqualified component.)
...
| `members`
| `string[]`
| The ids of the members of that scope (e.g., member functions, namespace symbols). Resolve each with `@root.mrdocs.corpus.get`.

| `overloads`
| `string[]`
| Same as `members`, but groups overloaded functions as unique symbols of kind `overload`. These are ids; resolve each with `@root.mrdocs.corpus.get`.
|===
...
| `usingDirectives`
| `string[]`
| The ids of the namespace's using directives. Resolve each with `@root.mrdocs.corpus.get`.
|===
...
| `id`
| `string`
| The id of the friend symbol (when a symbol is befriended). Resolve with `@root.mrdocs.corpus.get`.

| `type`
| <<type-info-fields,Type Object>>
| The friend type (when a type is befriended).
|===
...
| `shadows`
| `string[]`
| The ids of the symbols being used. Resolve each with `@root.mrdocs.corpus.get`.
...
| `string[]`
| The ids of the symbols of that kind in the scope. Resolve each with `@root.mrdocs.corpus.get`.

| `types`
| `string[]`
| The ids of the types in the scope. Resolve each with `@root.mrdocs.corpus.get`.

| `staticfuncs`
| `string[]`
| The ids of the static functions in the scope. Resolve each with `@root.mrdocs.corpus.get`.

| `overloads`
| `string[]`
| The ids of the overloads in the scope. Resolve each with `@root.mrdocs.corpus.get`.

| `staticoverloads`
| `string[]`
| The ids of the static overloads in the scope. Resolve each with `@root.mrdocs.corpus.get`.
|===
...
| `primary`
| `string`
| The id of the primary template. Resolve with `@root.mrdocs.corpus.get`.
...
Please refer to the xref:configuration/reference.adoc[configuration options reference] for more information.
