...
Lua is also faster, 2 to 20 times on the same work, and has no memory limit of its own, while each JavaScript engine has a fixed 4 GB heap. The xref:extensions/script-engines.adoc[Script Engines] page has the measurements and the details.
====
...
Each entry in `ctx.corpus.symbols` is a proxy for a live Mr.Docs symbol. The fields of each object are at xref:generators/reference.adoc[the DOM reference]. The array itself (internally represented by a set) is fixed in size: read it, iterate it, filter it, but copy the entries you keep into your own array or table before trimming or reordering them.
