...
the argument parser defined in `utils/bootstrap/src/__main__.py` and writes
the AsciiDoc partial that the install page includes. CI runs `--check` to
fail when the on-disk partial drifts from the parser.

Usage:
    python utils/docs/generate_bootstrap_options.py --update
    python utils/docs/generate_bootstrap_options.py --check
"""
import argparse
import os
import re
...
sys.path.insert(0, os.path.join(REPO_ROOT, "utils/bootstrap"))
...
HEADER = (
    ...
    "// Run `python utils/docs/generate_bootstrap_options.py --update` to "
    "regenerate.\n"
...
def main():
    ...
    if missing:
        print(
            "FAIL: utils/bootstrap/src/core/option_help.py is missing "
    ...
    if on_disk != rendered:
        print(
            ...
            f"`python utils/docs/generate_bootstrap_options.py --update`.",
            file=sys.stderr)
        return 1
    print("OK")
    return 0
