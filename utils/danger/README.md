...
- Danger executes via `npx danger ci --dangerfile utils/danger/dangerfile.ts` in the `Repo checks` job.
...
```bash
npm --prefix utils/danger ci           # install dev deps (without touching the repo root)
npm --prefix utils/danger test         # run Vitest unit tests for rule logic
npm --prefix utils/danger run danger:local  # print the fixture report from utils/danger/fixtures/sample-pr.json
npm --prefix utils/danger run danger:scope-map  # emit JSON mapping every tracked file to its Danger scope
npm --prefix utils/danger run danger:ci     # run Danger in CI mode (requires GitHub PR context)
```
