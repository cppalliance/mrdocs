...
def _preamble() -> list:
    ...
    return [
        "# Generated by bootstrap.py, and git-ignored on purpose: this file is",
        "# not committed and will not show up in `git status`. bootstrap.py",
        "# rewrites the managed recipes below on every run, matched by name (the",
        "# `bootstrap` recipe itself regenerates it). `just --list` shows everything.",
        "",
        _shell_setting(),
        _mrdocs_var_line(),
        "",
