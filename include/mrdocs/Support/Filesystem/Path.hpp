...
#ifndef MRDOCS_API_SUPPORT_FILESYSTEM_PATH_HPP
#define MRDOCS_API_SUPPORT_FILESYSTEM_PATH_HPP
...
/** Make a source path relative to the mrdocs project root.
    ...
    Strips the build-time project-root prefix so diagnostics print short,
    repository-relative paths (e.g. `src/mrdocs/Support/Report.cpp`). The root
    is derived from this translation unit's own location, so it needs no marker
    file at the repository root. A path that is not under the project root falls
    back to its bare file name.

    The result is a view into `pathName` (like @ref getFileName), so it is only
    valid while `pathName`'s storage lives.

    @param pathName An absolute source path, e.g. from `source_location`.
    @return The portion of `pathName` relative to the project root, or its bare
        file name when it is not under the root.
*/
MRDOCS_DECL
std::string_view
makeProjectRelative(
