...
#ifndef MRDOCS_API_SUPPORT_CHRONO_HPP
#define MRDOCS_API_SUPPORT_CHRONO_HPP
    ...
    std::chrono::duration<Rep, Period> delta)
{
    ...
    {
      return std::format("{}ms", delta_ms.count());
    }
    ...
    {
        delta_ms -= delta_s;
        return std::format("{}s {}ms", delta_s.count(), delta_ms.count());
    }
    ...
    if (delta < std::chrono::hours(1))
    {
        delta_ms -= delta_s;
        delta_s -= delta_min;
        return std::format("{}min {}s {}ms", delta_min.count(), delta_s.count(),
                           delta_ms.count());
    }
    ...
    delta_s -= delta_min;
    delta_min -= delta_h;
    return std::format("{}h {}min {}s {}ms", delta_h.count(), delta_min.count(),
                       delta_s.count(), delta_ms.count());
}
