...
Four checks:
  ...
  4. Every non-Planned entry has at least one example, and each example
     resolves to its golden-test mirror under test-files/golden-tests/
...
Usage:
    python util/check_commands_registry.py [--repo-root DIR]
...
def collect_dispatched_commands(root):
    ...
    """
    extract_path = os.path.join(root, "src/lib/AST/ExtractDocComment.cpp")
    parse_inlines_path = os.path.join(
        root, "src/lib/Metadata/Finalizers/DocComment/parseInlines.hpp"
    )
...
def check_examples_exist(entries, root):
    ...
    test-files/golden-tests/snippets/.
    ...
    """
    errors = []
    snippets_dir = os.path.join(root, "test-files/golden-tests/snippets")
        ...
        for ex in examples:
            ...
            if not os.path.isfile(mirror_cpp):
                errors.append(
                    ...
                    f"test-files/golden-tests/snippets/{ex}.cpp"
                )
            ...
            if not os.path.isfile(rendered):
                errors.append(
                    ...
                    f"(run `python util/render_snippets.py --update`)"
                )
    return errors
