...
def pick_default_preset(source_dir: str, active_preset: str) -> str:
    ...
    Reuses the scoring in util/run_all_tests.py so the default matches what
    `python util/run_all_tests.py` would pick. Falls back to the active
    preset when scoring is unavailable.
    """
    try:
        runner_path = os.path.join(source_dir, "util", "run_all_tests.py")
