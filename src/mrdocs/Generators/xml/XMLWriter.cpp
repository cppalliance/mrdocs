...
// A described object collapses to plain text when it has a single string
// member. Shared with the reflection DOM proxy (single source of truth).
template <typename T>
constexpr bool is_single_text_object = describe::isSingleStringObject<T>;
...
stripBaseName(std::string& name)
{
    if constexpr (describe::has_describe_bases<T>::value)
    {
        describe::for_each(describe::describe_bases<T>{}, [&](auto D) {
            ...
            std::string_view const base = unqualifiedTypeName<Base>();
...
std::string typeTag()
{
    std::string name(unqualifiedTypeName<T>());
