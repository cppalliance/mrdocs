...
#ifndef MRDOCS_LIB_GENERATORS_HBS_BUILDER_HPP
#define MRDOCS_LIB_GENERATORS_HBS_BUILDER_HPP
...
class Builder
{
    ...
    handlebars::Handlebars hbs_;
    std::map<std::string, std::string, std::less<>> templates_;
    std::function<void(handlebars::OutputRef&, std::string_view)> escapeFn_;

    // The `mrdocs` root object shared by every rendered page, exposed to
    // templates as `@root.mrdocs`. Its `corpus` member is the shared,
    // lazy corpus DOM (see buildCorpusDom) plus a generator-specific
    // `url` function; built once because it is O(symbols) to set up.
    dom::Value mrdocs_;
...
#endif // MRDOCS_LIB_GENERATORS_HBS_BUILDER_HPP
