...
/** Makes a URL relative to another URL.
    ...
    When called with a single argument, the current page URL (from
    `data.root.page.url`) is used as the source path.
...
relativize_fn(dom::Value to0, dom::Value from0, dom::Value options)
{
    ...
    if (singleArg)
    {
        ...
        options = from0;
        from0 = options.lookup("data.root.page.url");
    }
...
*/
void
registerPartials(handlebars::Handlebars& hbs, std::vector<std::string> const& dirs)
{
...
*/
void
registerDefaultHelpers(handlebars::Handlebars& hbs)
{
    ...
    handlebars::helpers::registerConstructorHelpers(hbs);
    handlebars::helpers::registerStringHelpers(hbs);
    handlebars::helpers::registerAntoraHelpers(hbs);
    handlebars::helpers::registerLogicalHelpers(hbs);
    handlebars::helpers::registerMathHelpers(hbs);
    handlebars::helpers::registerContainerHelpers(hbs);
    handlebars::helpers::registerTypeHelpers(hbs);
...
Builder(
    ...
    std::function<void(handlebars::OutputRef&, std::string_view)> escapeFn)
    ...
    , domCorpus(corpus)
{
    ...
    // The format-inheritance chain: this generator's format id plus the
    // ids it extends, most-general first (e.g. {"adoc", "md"} for a `md`
    // generator that extends `adoc`). Partials and helpers are searched
    // along it so a derived format inherits its base's addons.
    auto const formatChain = addon_paths::extensionChain(domCorpus.fileExtension);
    auto const partialDirs = addon_paths::partialDirs(roots, formatChain);
    auto const helperDirs = addon_paths::helperDirs(roots, formatChain);
    ...
    // Build the shared `mrdocs` root object once. `mrdocs.corpus` is the
    // reflection-driven corpus DOM (symbols/get/lookup, see
    // buildCorpusDom), to which the generator adds a `url` function:
    // given a symbol object it returns the (absolute) href to that
    // symbol's page, which is presentation logic (output mode,
    // primary-vs-dependency) that only the generator knows. `relativize`
    // still turns the result relative to the page being rendered.
    // `getUrl` computes a symbol's page URL.
    dom::Value corpusVal = buildCorpusDom(domCorpus.getCorpus());
    corpusVal.getObject().set("getUrl", dom::makeInvocable(
        [this](dom::Value const& symV) -> dom::Value
        {
            if (!symV.isObject())
            {
                return nullptr;
            }
            dom::Value const idV = symV.get("id");
            if (!idV.isString())
            {
                return nullptr;
            }
            std::optional<SymbolID> const id =
                fromBase58Str(idV.getString());
            if (!id)
            {
                return nullptr;
            }
            ...
            return domCorpus.getURL(*sym);
        }));
    // `qualifiedAnchor` returns the qualified heading/fragment id for a
    // symbol, named to match getQualifiedAnchor. Symbol.Anchor stores only
    // the unqualified name, so the qualified id (which must be unique on a
    // page) is assembled here. It stays a helper rather than a Handlebars
    // partial because a heading id is a hash-argument attribute value
    // (markup/h `id=`), and a partial can only render content, not produce
    // a value. It is not redundant with `getUrl`: an id is the '-'-joined
    // qualified name, whereas a multipage url is a '/'-joined path with a
    // file extension and a single-page url has a '#' prefix; both derive
    // from getQualifiedAnchor, differing only in delimiter and affixes.
    corpusVal.getObject().set("qualifiedAnchor", dom::makeInvocable(
        [this](dom::Value const& symV) -> dom::Value
        {
            if (!symV.isObject())
            {
    ...
    mrdocs_ = dom::Value(std::move(mrdocsObj));
    ...
    dom::Value const& context)
{
    ...
    handlebars::HandlebarsOptions options;
    options.escapeFunction = escapeFn_;
    handlebars::OutputRef out(os);
    Expected<void, handlebars::Error> exp =
...
createContext(Symbol const& I)
{
    ...
    // The URL of the page currently being rendered. `relativize` reads it
    // from `data.root.page.url` to make hrefs relative to this page.
    page.set("url", domCorpus.getURL(I));
    ctx.set("page", page);
    ctx.set("symbol", domCorpus.get(I.id));
    ctx.set("mrdocs", mrdocs_);
    ctx.set("config", describedToDom(domCorpus.config()));
...
operator()(std::ostream& os, T const& I)
{
  ...
  dom::Object const ctx = createContext(I);
    ...
    dom::Object const wrapperCtx = handlebars::createFrame(ctx);
    wrapperCtx.set("contents", dom::makeInvocable([this, templateFile, ctx, &os](
        dom::Value const&) -> dom::Expected<dom::Value>
        {
