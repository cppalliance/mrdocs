...
struct ConfigSchemaVisitor {
    ...
    /*  Note an option marked as deprecated, for
        ...
        the reporting level is forced to errors-only.
    */
    void
    recordIfDeprecated(
...
normalize(ReferenceDirectories const& dirs)
{
    ...
    // `tagfile` is what `output-tagfile` was called. Both have been
    // normalized by now, so this hands over a finished path.
    if (!tagfile.empty())
    {
        outputTagfile = tagfile;
    }
    return {};
}
