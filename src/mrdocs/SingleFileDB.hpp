...
/** Compilation database for a single file.
    ...
    A small helper for driving extraction over one source file, used by the
    test harnesses. The tool builds its compilation database from the
    configuration instead (see `MrDocsCompilationDatabase`).
*/
class SingleFileDB
...
#endif // MRDOCS_LIB_SINGLEFILEDB_HPP
