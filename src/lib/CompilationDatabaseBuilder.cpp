...
*/
Expected<std::string>
resolveCompilationDatabasePath(Config::Settings const& settings)
{
...
resolveCMakeBuildLocation(Config::Settings const& settings)
{
    ...
    std::string compileCommandsPath)
{
    ...
    MRDOCS_TRY(
        std::string content,
        readCompileCommands(compileCommandsPath, config->settings().sourceRoot));
    ...
    std::shared_ptr<ConfigImpl const> const& config)
{
    ...
    MRDOCS_TRY(
        ...
        resolveCompilationDatabasePath(config->settings()));
