...
generateAndCompareOutput(CompareArgs const& args)
{
    ...
    std::string const outputDir = getGeneratorOutputPath(args.gen, args.config);
    bool const singlePage = !args.config.multipage;
    ...
    if (auto exp = args.gen.build(args.corpus, args.config); !exp)
    {
